...
def test_the_institutional_hop_is_registered():
    assert "corpus_institutional_search" in {t.name for t in _listed_tools()}
...
def test_institutional_search_reports_failure_rather_than_an_empty_shelf(monkeypatch):
    monkeypatch.setattr(
        ...
        lambda q, **k: {"hits": [], "ok": False, "base_url": "https://r",
...
def test_institutional_search_narrows_and_limits(monkeypatch):
    seen = {}

    def _search(q, *, sources=None, limit_per_source=3):
        seen.update(query=q, sources=sources, limit_per_source=limit_per_source)
        return {"hits": [{"n": i} for i in range(20)], "ok": True,
                "base_url": "https://r", "sources_queried": ["arxiv"],
                "total": 20, "error": ""}
    ...
    assert seen == {"query": "q", "sources": ["arxiv"], "limit_per_source": 2}
...
def test_search_status_covers_both_outward_hops(monkeypatch):
    ...
    assert status["institutional"]["configured"] is False
    assert "JELES_REMOTE_SECRET" in status["institutional"]["reason"]


def test_the_confidence_ladder_has_four_distinct_rungs(monkeypatch):
